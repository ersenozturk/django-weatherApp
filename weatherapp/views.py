...
def base(request):
    ...
    if u_city:
        ...
        if response.ok:  # 400 ün altında true olarak döner
            ...
            # cityInfo içindeki  name api denn geldi
            r_city = cityInfo['name']
    ...
    for each_city in cities:
        # db deki bütün cityleri döngüye aldık
        url = f'https://api.openweathermap.org/data/2.5/weather?q={each_city}&appid={API_key}&units=metric&'
        response = requests.get(url)
